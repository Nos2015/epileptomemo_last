...
@Component({
  selector: 'app-memorysequencegame',
  standalone: false,
  templateUrl: './memorysequencegame.component.html',
  styleUrl: './memorysequencegame.component.scss'
})
export class MemorysequencegameComponent implements OnInit{
  green = "";
  red = "";
  blue = "";
  yellow = "";
  ...
  @Output() joinUsExit = new EventEmitter<boolean>();
  @Output() quitUsExit = new EventEmitter<boolean>();
  ...
  newOrEndGame(){
    console.log("MemorysequenceComponent newOrEndGame numbertimesplayed : " + this.numbertimesplayed);
  ...
  selectColor(color:string){
    ...
    if(this.playerSequence.length == this.sequence.length){
      ...
      this.setOrNotSetBestScore();
      this.win = true;
      setTimeout(() => this.newOrEndGame(), 1000);
    }
  }
  ...
  setTimesPlayed(){
    ...
    console.log("MemorysequenceComponent setTimesPlayed numbertimesplayed : " + this.numbertimesplayed);
  }

  checkUserCanPlay():boolean{
    let timesplayed = this.getTimesPlayed();
    console.log("MemorysequenceComponent checkUserCanPlay numbertimesplayed : " + timesplayed);
    if(timesplayed < 3){
        return true;
    }
    else{
  ...
  join(){
    this.joinUsExit.emit(false);
  }

  quit(){
    this.quitUsExit.emit(false);
  }
